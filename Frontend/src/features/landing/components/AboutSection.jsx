...
function AboutSection() {
  return (
    <section className="py-16 md:py-24 text-foreground relative overflow-hidden">
      {" "}
      {/* Increased padding for spaciousness, removed bg-background here */}
      {/* NEW: Linear Background Gradient (from site-gradient vars) - Mimics body background */}
      <div
        className="absolute inset-0 z-0 opacity-25" // Opacity from FeatureSection
        style={{
          background:
            "linear-gradient(to bottom, var(--site-gradient-start) 0%, var(--site-gradient-via) 50%, var(--site-gradient-end) 100%)",
          backgroundAttachment: "fixed", // Keep fixed like body background
        }}
      ></div>
      {/* NEW: Dotted Background Pattern (from body::before) - Consistent with FeatureSection */}
      <div
        className="absolute inset-0 z-0 opacity-5" // Opacity from body::before
        style={{
          backgroundImage:
      ...
      ></div>
      {/* NEW: Radial Gradient Background (like FeatureSection's subtle internal gradient) */}
      ...
      <div className="container mx-auto px-4 relative z-10 text-center max-w-4xl">
        {" "}
        {/* Content container */}
        {/* Heading - Matching FeatureSection's h2 style */}
        <h2 className="text-4xl md:text-5xl font-extrabold mb-8 leading-tight">
          About{" "}
          <span className="text-blue-600 dark:text-yellow-400">CodeRise</span>{" "}
          {/* Used specific colors for highlight */}
        </h2>
        {/* Paragraphs - Matching FeatureSection's p style */}
        ...
        {/* Button - Matching FeatureSection's main CTA button style */}
